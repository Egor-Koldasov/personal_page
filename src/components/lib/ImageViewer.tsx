...
const ImageInline = (props: ImageInlineProps) => {
  ...
  return (
    ...
    >
      {containerHeight > 0 && (
        <Image src={props.src} alt="Image" objectFit="contain" layout="fill" />
      )}
    </ImageInlineStyled>
  )
}
...
const ImageInlineList = forwardRef(
  (props: ScrollListProps, externalRef: Ref<HTMLDivElement>) => {
    const ref = useRef<HTMLDivElement>(null)
    const [autoScrollDir, setAutoScrollDir] = useState<"left" | "right">(
      "right"
    )
    ...
    const runAutoScrollStep = useCallback(() => {
      ...
      if (!el || el.scrollWidth <= el.clientWidth || isHovering || isPaused)
        return
...
export const ModalCloseBtn = styled.button`
  background-color: ${theme((t) => t.sectionPortfolio.imageCloseButtonBg)};
  &:hover {
    background-color: ${theme((t) => t.sectionPortfolio.imageCloseButtonBg)};
  }
  &::before,
  &::after {
    background-color: ${theme((t) => t.sectionPortfolio.imageCloseButtonColor)};
  }
`
